...
class OfficeParser(object):
    ...
    def timestamp_string(self, wtimestamp):
        timestamp = (old_div(wtimestamp, 10000000)) - 11644473600
        if timestamp > 0:
            datestring = time.strftime("%Y/%m/%d %H:%M:%S", time.gmtime(timestamp))
        else:
            timestamp = (old_div(wtimestamp, 10000000))
            datestring = "%02d:%02d:%02d" % (old_div(timestamp, 360), old_div(timestamp, 60), timestamp % 60) 
    ...
    def parse_directory(self, data):
        if len(data) >= 128:
            ...
            entry = {
                ...
                'object_type':      struct.unpack('B', data[66])[0],
                'color':            struct.unpack('B', data[67])[0],
            ...
            norm_name = entry['name'][:name_len].replace('\x00','')
            # check for known subtype headers indicating special objects
            if len(norm_name) >= 1:
                if norm_name[0] in ['\x01', '\x03', '\x05']:
